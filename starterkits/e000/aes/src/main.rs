...
// NOTE: Current v2 header occupies 62 bytes; last 2 bytes are reserved (zeroed).
const FLAG_KEY_SOURCE_ARGON2: u16 = 0x0001;

const ARGON2_M_COST_KIB: u32 = 256 * 1024; // 256 MiB
const ARGON2_T_COST: u32 = 2;

// Hard caps for *untrusted* Argon2 params parsed from headers (DoS protection).
const MAX_ARGON2_M_COST_KIB: u32 = 1024 * 1024; // 1 GiB
...
fn validate_argon2_params(m_cost_kib: u32, t_cost: u32, lanes: u32) -> Result<()> {
    if m_cost_kib == 0 || m_cost_kib > MAX_ARGON2_M_COST_KIB {
        bail!("Argon2 m_cost_kib out of range (1..={})", MAX_ARGON2_M_COST_KIB);
    }
    if t_cost == 0 || t_cost > MAX_ARGON2_T_COST {
        bail!("Argon2 t_cost out of range (1..={})", MAX_ARGON2_T_COST);
    }
    if lanes == 0 || lanes > MAX_ARGON2_LANES {
        bail!("Argon2 lanes out of range (1..={})", MAX_ARGON2_LANES);
    }
    Ok(())
}
...
impl V2Header {
    fn to_bytes(&self) -> [u8; V2_HEADER_LEN] {
        // NOTE: two trailing bytes remain zero (reserved for future use).
        ...
        // bytes [off..V2_HEADER_LEN) remain zeroed (reserved)
        debug_assert_eq!(off, 62);
        buf
    }
    ...
    fn parse(buf: &[u8]) -> Result<Self> {
        ...
        // Verify reserved trailing bytes are zero (defensive parsing)
        if buf[off..V2_HEADER_LEN].iter().any(|&b| b != 0) {
...
fn main() -> Result<()> {
    ...
    let head_read: usize = {
        ...
        rdr.read(&mut head)
            .with_context(|| "failed to read header preview")?
    };
    ...
    match mode {
        Mode::Encrypt => {
            ...
            if let Some(kp) = &opt.keyfile {
                ...
                let kb = Zeroizing::new(
                    fs::read(&default_key_path).with_context(|| {
            ...
            // cipher dropped here; key was zeroized in encrypt_v2
        }
    ...
    #[cfg(unix)]
    {
        // On Unix, NamedTempFile::persist() overwrites atomically.
...
fn decrypt_v2(
    ...
    if (hdr.flags & FLAG_KEY_SOURCE_ARGON2) != 0 {
        validate_argon2_params(hdr.m_cost_kib, hdr.t_cost, hdr.lanes)?;
        let password = if let Ok(env_pw) = std::env::var("AES_PASSWORD") {
            Zeroizing::new(env_pw)
        } else {
            Zeroizing::new(prompt_password("Password: ").context("failed to read password")?)
