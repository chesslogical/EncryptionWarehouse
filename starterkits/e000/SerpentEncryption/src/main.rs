...
const IV_LEN: usize = 16; // Serpent block size is 16 bytes
...
// Bounds to prevent header-driven KDF DoS.
const MIN_ARGON2_M_KIB: u32 = 8 * 1024; // 8 MiB
const MAX_ARGON2_M_KIB: u32 = 1_048_576; // 1 GiB
const MIN_ARGON2_T: u32 = 1;
const MAX_ARGON2_T: u32 = 10;
const MIN_ARGON2_P: u32 = 1;

// HKDF salt (context tag) for domain separation (not secrecy).
const HKDF_SALT: &[u8] = b"serpent2/v1";
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Commands::Encrypt {
            ...
            // Use SecretString so password memory is wiped on drop.
            let pwd = SecretString::new(prompt_password("Enter password: ")?);
            let confirm = SecretString::new(prompt_password("Confirm password: ")?);
            if pwd.expose_secret() != confirm.expose_secret() {
                anyhow::bail!("passwords did not match");
            }
            drop(confirm); // wipe confirm now

            let salt = random_bytes(SALT_LEN);
            let master = derive_key_with_params(pwd.expose_secret(), &salt, ARGON2_M_KIB, ARGON2_T, ARGON2_P)?;
            let (k_enc, k_auth) = derive_subkeys(&master)?;

            let data = fs::read(&input).context("reading input file")?;
            let iv = random_bytes(IV_LEN);

            // Encrypt (CBC + PKCS#7 in-place)
            let mut buf = data; // move, don't clone
            let msg_len = buf.len();
            ...
            // Wipe plaintext buffers eagerly.
            buf.zeroize();
        ...
        Commands::Decrypt {
            ...
            validate_kdf_params(m_kib, t, p)?; // guard before Argon2
            ...
            let master = derive_key_with_params(pwd.expose_secret(), salt, m_kib, t, p)?;
            let (k_enc, k_auth) = derive_subkeys(&master)?;
...
fn validate_kdf_params(m_kib: u32, t: u32, p: u32) -> Result<()> {
    if m_kib < MIN_ARGON2_M_KIB || m_kib > MAX_ARGON2_M_KIB {
        anyhow::bail!("unsupported Argon2 memory: {} KiB", m_kib);
    }
    if t < MIN_ARGON2_T || t > MAX_ARGON2_T {
        anyhow::bail!("unsupported Argon2 iterations: {}", t);
    }
    let max_p = num_cpus::get().max(1) as u32;
    if p < MIN_ARGON2_P || p > max_p {
        anyhow::bail!("unsupported Argon2 parallelism: {} (max {})", p, max_p);
    }
    Ok(())
}
...
fn derive_key_with_params(
    ...
    // Pin Argon2 output length to 32 bytes.
    ...
    argon2
        ...
        .map_err(|e| anyhow::anyhow!("Argon2 derive failed: {e}"))?;
    Ok(Secret::new(key))
}

fn derive_subkeys(master: &Secret<[u8; 32]>) -> Result<(Secret<[u8; 32]>, Secret<[u8; 32]>)> {
    // Domain separation via HKDF salt + per‑purpose info labels.
    let hk = Hkdf::<Sha256>::new(Some(HKDF_SALT), master.expose_secret());
