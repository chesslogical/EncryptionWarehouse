...
use windows_sys::Win32::Storage::FileSystem::{
    ReplaceFileW, REPLACEFILE_IGNORE_MERGE_ERRORS, FILE_FLAG_OPEN_REPARSE_POINT,
};
...
fn atomic_replace(src: &Path, dest: &Path) -> Result<()> {
    ...
    // Replace *dest* with *src*. Note: REPLACEFILE_WRITE_THROUGH is not supported by ReplaceFileW.
    ...
    // Strengthen durability: open the replaced file for write and flush.
    // If the file was read-only, temporarily clear the bit to obtain a write handle.
    let mut perms = fs::metadata(dest)?.permissions();
    let was_ro = perms.readonly();
    if was_ro {
        perms.set_readonly(false);
        fs::set_permissions(dest, perms.clone())?;
    }

    let f = OpenOptions::new()
        .write(true)
        .open(dest)
        .with_context(|| format!("open {:?} to flush", dest))?;
    f.sync_all().context("sync replaced file (FlushFileBuffers)")?;
    drop(f);

    // Restore read-only bit if it was set before.
    if was_ro {
        let mut perms2 = fs::metadata(dest)?.permissions();
        perms2.set_readonly(true);
...
fn fsync_dir_of(_path: &Path) -> Result<()> {
    // No portable directory flush on Windows.
    Ok(())
}
...
fn open_file_nofollow_read(path: &Path) -> Result<File> {
    ...
    // Open the reparse point itself (no-follow). We'll reject if it's actually a RP.
    let f = std::fs::OpenOptions::new()
        .read(true)
...
fn encrypt_in_place(path: &Path) -> Result<()> {
    ...
    // Build AEAD — chacha20poly1305 depends on `zeroize`, so keying material is wiped on Drop.
...
fn decrypt_in_place(path: &Path) -> Result<()> {
    ...
    let body_len = total_len
        ...
        .ok_or_else(|| anyhow!("ciphertext truncated"))?;
    let total_chunks = chunks_for_len(header.plaintext_len, header.chunk_size);
    let tags = (AEAD_TAG_LEN as u64)
        .checked_mul(total_chunks)
        .ok_or_else(|| anyhow!("overflow computing expected ciphertext size"))?;
    let expected_body = header
        .plaintext_len
        .checked_add(tags)
        .ok_or_else(|| anyhow!("overflow computing expected ciphertext size"))?;
    ensure!(
        body_len == expected_body,
        "ciphertext length mismatch (expected {}, found {})",
        expected_body,
        body_len
    );
    ...
    let key = Key::from_slice(&*k_enc);
    {
        ...
        // On Windows, ReplaceFileW preserves the target's attributes.
        // If we *didn't* request preserve, enforce our desired perms now.
    #[cfg(windows)]
{
    if env_or("SFE_PRESERVE_PERMS", 0u8) == 0 {
...
fn verify_only(path: &Path) -> Result<()> {
    ...
    if header.alg_id != ALG_XCHACHA20POLY1305 || header.kdf_id != KDF_ARGON2ID {
        ...
        header.chunk_size > 0 && (header.chunk_size as usize) <= MAX_CHUNK_SIZE,
        "invalid or unsafe chunk size"
    );
    ...
    let max_p = env_or("SFE_MAX_P", DEFAULT_MAX_ARGON2_P);
        ...
        header.m_cost_kib <= max_m
            && header.t_cost >= 1
            && header.t_cost <= max_t
            && header.p_cost >= 1
            && header.p_cost <= max_p,
        "file requires Argon2 params outside allowed caps"
    );
    ...
    let tags = (AEAD_TAG_LEN as u64)
        .checked_mul(chunks)
        .ok_or_else(|| anyhow!("overflow computing expected size"))?;
    let expected_body = header
        .plaintext_len
        .checked_add(tags)
        .ok_or_else(|| anyhow!("overflow computing expected size"))?;
    ...
    let key = Key::from_slice(&*k_enc);
    {
        ...
        while index < chunks {
            ...
            aead
                .decrypt_in_place_detached(&nonce, &aad, &mut buf[..this_plain], tag)
