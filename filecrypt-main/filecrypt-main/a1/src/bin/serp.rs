...
// The design choice of requiring the serp.key file in the current working directory is intentional.
...
const DEFAULT_KEY_FILE: &str = "serp.key";
...
fn encrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    if path.file_name().map_or(false, |name| name == "serp.key") {
