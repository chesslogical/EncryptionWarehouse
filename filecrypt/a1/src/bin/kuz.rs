...
// The design choice of requiring the kuz.key file in the current working directory is intentional.
...
const DEFAULT_KEY_FILE: &str = "kuz.key";
const ABYTES: usize = 64; // HMAC-SHA512 tag size
const HEADERBYTES: usize = 16; // IV size
const KEYBYTES: usize = 32; // Kuznechik key is 256 bits = 32 bytes
const MAC_KEYBYTES: usize = 64;
const BLOCK_SIZE: usize = 16; // Kuznechik block size is 128 bits = 16 bytes
#[derive(Parser)]
#[command(name = "kuz")]
#[command(about = "Simple file encryption CLI using Kuznechik (GOST R 34.12-2015)")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn encrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    if path.file_name().map_or(false, |name| name == "kuz.key") {
...
fn ctr_process(enc_key: &[u8], iv: &[u8], data: &[u8], mut block_counter: u64) -> Result<Vec<u8>> {
    ...
    let cipher = Kuznyechik::new_from_slice(&enc_key_arr).map_err(|_| anyhow!("Cipher init failed"))?;
