...
const KEYBYTES: usize = 32; // Serpent-256 key is 256 bits = 32 bytes
const MAC_KEYBYTES: usize = 64;
const BLOCK_SIZE: usize = 16; // Serpent block size is 128 bits = 16 bytes
...
fn decrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        while remaining_ct > 0 {
            ...
            let n = input.read(&mut ct[..to_read])?;
            if n == 0 {
                return Err(anyhow!("Unexpected EOF during ciphertext read"));
            }
            mac.update(&ct[..n]);
            remaining_ct -= n;
        }
...
fn pw_decrypt(file: &str) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        let file_len = metadata.len() as usize;
        if file_len < MAGIC_PW.len() + PWHASH_SALTBYTES + HEADERBYTES + ABYTES {
            return Err(anyhow!("File too short for encrypted data"));
        }
        let ct_len = file_len - MAGIC_PW.len() - PWHASH_SALTBYTES - HEADERBYTES - ABYTES;
        let mut input = BufReader::new(input_file);
        let mut magic_buf = [0u8; 4];
...
fn ctr_process(enc_key: &[u8], iv: &[u8], data: &[u8], mut block_counter: u64) -> Result<Vec<u8>> {
    ...
    let cipher = Serpent::new_from_slice(&enc_key_arr).map_err(|_| anyhow!("Cipher init failed"))?;
