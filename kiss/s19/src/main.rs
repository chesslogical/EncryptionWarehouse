...
fn encrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        loop {
            ...
            let plain_chunk = plain[..n].to_vec();
        ...
        let final_ct = push_stream.push_to_vec(&empty, None, Tag::FINAL).map_err(|_| anyhow!("Encryption failed on final tag"))?;
