...
fn derive_nonce(base: &[u8], counter: u32) -> [u8; NONCEBYTES] {
    let mut nonce = [0u8; NONCEBYTES];
    nonce[0..HEADERBYTES].copy_from_slice(base);
    nonce[HEADERBYTES..NONCEBYTES].copy_from_slice(&counter.to_be_bytes());
    nonce
}
...
fn encrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        let mut base_nonce: StackByteArray<{ HEADERBYTES }> = StackByteArray::new();
        copy_randombytes(&mut base_nonce);
        ...
        loop {
            ...
            let aad = [counter.to_le_bytes().as_slice(), &[0u8]].concat();
            let nonce_arr = derive_nonce(base_nonce.as_slice(), counter);
            let nonce = Nonce::from_slice(&nonce_arr);
            let key_g = aes_gcm_siv::Key::<Aes256GcmSiv>::from_slice(key.as_slice());
            let cipher = Aes256GcmSiv::new(key_g);
            let ct = cipher
                .encrypt(nonce, Payload { msg: &plain[..n], aad: &aad })
        ...
        // Append final frame
        let aad = [counter.to_le_bytes().as_slice(), &[1u8]].concat();
        let nonce_arr = derive_nonce(base_nonce.as_slice(), counter);
        let nonce = Nonce::from_slice(&nonce_arr);
        let key_g = aes_gcm_siv::Key::<Aes256GcmSiv>::from_slice(key.as_slice());
        let cipher = Aes256GcmSiv::new(key_g);
        let ct = cipher
            ...
            .map_err(|_| anyhow!("Encryption failed on final tag"))?;
...
fn decrypt(file: &str, key: &StackByteArray<KEYBYTES>) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        let base_nonce: [u8; HEADERBYTES] = header_buf;
        let output_file = OpenOptions::new()
            .write(true)
            .create_new(true)
            .custom_flags(libc::O_NOFOLLOW)
        ...
        loop {
            ...
            let aad = [counter.to_le_bytes().as_slice(), &[0u8]].concat();
            let nonce_arr = derive_nonce(&base_nonce, counter);
            let nonce = Nonce::from_slice(&nonce_arr);
            let key_g = aes_gcm_siv::Key::<Aes256GcmSiv>::from_slice(key.as_slice());
            let cipher = Aes256GcmSiv::new(key_g);
            let payload = Payload { msg: &ct[..ct_len], aad: &aad };
            match cipher.decrypt(nonce, payload) {
                ...
                Err(_) => {
                    // Try with final tag
                    let aad_final = [counter.to_le_bytes().as_slice(), &[1u8]].concat();
                    let nonce = Nonce::from_slice(&nonce_arr);
                    let payload_final = Payload { msg: &ct[..ct_len], aad: &aad_final };
                    let mut plain = cipher.decrypt(nonce, payload_final).map_err(|_| anyhow!("Decryption failed (invalid MAC or data)"))?;
                    if plain.len() != plain_len {
                        return Err(anyhow!("Length mismatch after decryption"));
                    }
                    if plain_len != 0 {
                        return Err(anyhow!("Final frame must be empty"));
                    }
                    plain.zeroize();
                    seen_final = true;
...
fn load_key_from_file(key_file: &str) -> Result<StackByteArray<KEYBYTES>> {
    let mut key_bytes = fs::read(key_file).context(format!("Failed to read {}", key_file))?;
    if key_bytes.len() != KEYBYTES {
        return Err(anyhow!("{} must be exactly {} bytes", key_file, KEYBYTES));
    }
    let mut key: StackByteArray<KEYBYTES> = StackByteArray::new();
